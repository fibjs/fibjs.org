...
It is created via the [MessageChannel](MessageChannel.md) constructor, which creates a pair of connected ports.
...
--------------------------
### start
**Start receiving messages queued on this port**
...
--------------------------
### close
**Close the port. No more messages can be sent or received after closing.**
...
### message
**查询和绑定接受消息事件，相当于 on("message", func); 设置后会自动调用 start()。**

```JavaScript
event MessagePort.message();
```

--------------------------
### messageerror
**查询和绑定接受消息反序列化错误事件，相当于 on("messageerror", func);**
