# 对象 ZlibCodec
ZlibCodec 是 [zlib](../../module/ifs/zlib.md) 压缩解压缩编解码器的基类，提供 [zlib](../../module/ifs/zlib.md) 类构造函数接口

ZlibCodec 继承自 [EventEmitter](EventEmitter.md)，可被用于 minizlib 等 npm 包。子类包括 [Gzip](Gzip.md)、[Gunzip](Gunzip.md)、[Deflate](Deflate.md)、[Inflate](Inflate.md)、[DeflateRaw](DeflateRaw.md)、[InflateRaw](InflateRaw.md)、[Unzip](Unzip.md)。
...
digraph {
    ...
    ZlibCodec [tooltip="ZlibCodec", fillcolor="lightgray", id="me", label="{ZlibCodec|_handle\l|_processChunk()\lclose()\lreset()\lparams()\l}"];
...
```JavaScript
static Object ZlibCodec.addAbortListener(EventEmitter signal,
    Function func);
```
...
```JavaScript
static Object ZlibCodec.once(EventEmitter emitter,
    Value ev,
    Object options = {});
```
...
```JavaScript
static Object ZlibCodec.on(EventEmitter emitter,
    Value ev,
    Object options = {});
```
...
```JavaScript
static Integer ZlibCodec.defaultMaxListeners;
```
...
```JavaScript
Buffer ZlibCodec._processChunk(Buffer chunk,
    Integer flushFlag);
```
...
```JavaScript
ZlibCodec.params(Integer level,
    Integer strategy);
```
...
```JavaScript
Object ZlibCodec.on(Object map);
```
...
```JavaScript
Object ZlibCodec.addListener(Value ev,
    Function func);
```
...
```JavaScript
Object ZlibCodec.addListener(Object map);
```
...
```JavaScript
Object ZlibCodec.addEventListener(Value ev,
    Function func,
    Object options = {});
```
...
```JavaScript
Object ZlibCodec.prependListener(Value ev,
    Function func);
```
...
```JavaScript
Object ZlibCodec.prependListener(Object map);
```
...
```JavaScript
Object ZlibCodec.once(Value ev,
    Function func);
```
...
```JavaScript
Object ZlibCodec.once(Object map);
```
...
```JavaScript
Object ZlibCodec.prependOnceListener(Value ev,
    Function func);
```
...
```JavaScript
Object ZlibCodec.prependOnceListener(Object map);
```
...
```JavaScript
Object ZlibCodec.off(Object map);
```
...
```JavaScript
Object ZlibCodec.removeListener(Value ev,
    Function func);
```
...
```JavaScript
Object ZlibCodec.removeListener(Object map);
```
...
```JavaScript
Object ZlibCodec.removeEventListener(Value ev,
    Function func,
    Object options = {});
```
...
```JavaScript
Object ZlibCodec.removeAllListeners(Value ev);
```
...
```JavaScript
Object ZlibCodec.removeAllListeners(Array evs = []);
```
...
```JavaScript
ZlibCodec.setMaxListeners(Integer n);
```
...
```JavaScript
Integer ZlibCodec.getMaxListeners();
```
...
```JavaScript
Array ZlibCodec.listeners(Value ev);
```
...
```JavaScript
Array ZlibCodec.rawListeners(Value ev);
```
...
```JavaScript
Integer ZlibCodec.listenerCount(Value ev);
```
...
```JavaScript
Integer ZlibCodec.listenerCount(Value o,
    Value ev);
```
...
```JavaScript
Boolean ZlibCodec.emit(Value ev,
    ...args);
```
...
```JavaScript
Value ZlibCodec.toJSON(String key = "");
```
