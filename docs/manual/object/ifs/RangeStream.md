...
也可以对普通 [Stream](Stream.md) 对象进行长度限制读取，此时 begin 固定为 0，仅限制读取的字节数：
...
如果传入的 stm 是 [SeekableStream](SeekableStream.md)，则等同于 RangeStream(stm, 0, end)。
...
--------------------------
**RangeStream 构造函数, 用于对普通 [Stream](Stream.md) 进行长度限制读取**

```JavaScript
new RangeStream(Stream stm,
    Long end);
```

调用参数:
* stm: [Stream](Stream.md), 基础流对象, 如果是 [SeekableStream](SeekableStream.md) 则等同于 RangeStream(stm, 0, end)
* end: Long, 从 stm 读取内容的最大字节数
...
```JavaScript
static Object RangeStream.addAbortListener(EventEmitter signal,
    Function func);
```
...
```JavaScript
static Object RangeStream.once(EventEmitter emitter,
    Value ev,
    Object options = {});
```
...
```JavaScript
static Object RangeStream.on(EventEmitter emitter,
    Value ev,
    Object options = {});
```
...
```JavaScript
static Integer RangeStream.defaultMaxListeners;
```
...
```JavaScript
readonly Boolean RangeStream.writable;
```
...
```JavaScript
readonly Boolean RangeStream.readable;
```
...
```JavaScript
readonly Object RangeStream._readableState;
```
...
```JavaScript
readonly Object RangeStream._writableState;
```
...
```JavaScript
Variant RangeStream.read(Integer bytes = -1) async;
```
...
```JavaScript
Buffer RangeStream.readBuffer(Integer bytes = -1) async;
```
...
```JavaScript
Stream RangeStream.setEncoding(String encoding);
```
...
```JavaScript
RangeStream.writeBuffer(Buffer data) async;
```
...
```JavaScript
Boolean RangeStream.write(Buffer data) async;
```
...
```JavaScript
Boolean RangeStream.write(Buffer data,
...
```JavaScript
Boolean RangeStream.write(String data,
...
```JavaScript
Value RangeStream.pipe(Value destination,
    Object options = {});
```
...
```JavaScript
RangeStream.unpipe(Stream destination = NULL);
```
...
```JavaScript
StreamReader RangeStream.getReader();
```
...
```JavaScript
Stream RangeStream.destroy(Value err = undefined) async;
```
...
```JavaScript
Object RangeStream.on(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.on(Object map);
```
...
```JavaScript
Object RangeStream.addListener(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.addListener(Object map);
```
...
```JavaScript
Object RangeStream.addEventListener(Value ev,
    Function func,
    Object options = {});
```
...
```JavaScript
Object RangeStream.prependListener(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.prependListener(Object map);
```
...
```JavaScript
Object RangeStream.once(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.once(Object map);
```
...
```JavaScript
Object RangeStream.prependOnceListener(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.prependOnceListener(Object map);
```
...
```JavaScript
Object RangeStream.off(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.off(Object map);
```
...
```JavaScript
Object RangeStream.removeListener(Value ev,
    Function func);
```
...
```JavaScript
Object RangeStream.removeListener(Object map);
```
...
```JavaScript
Object RangeStream.removeEventListener(Value ev,
    Function func,
    Object options = {});
```
...
```JavaScript
Object RangeStream.removeAllListeners(Value ev);
```
...
```JavaScript
Object RangeStream.removeAllListeners(Array evs = []);
```
...
```JavaScript
RangeStream.setMaxListeners(Integer n);
```
...
```JavaScript
Integer RangeStream.getMaxListeners();
```
...
```JavaScript
Array RangeStream.listeners(Value ev);
```
...
```JavaScript
Array RangeStream.rawListeners(Value ev);
```
...
```JavaScript
Integer RangeStream.listenerCount(Value ev);
```
...
```JavaScript
Integer RangeStream.listenerCount(Value o,
    Value ev);
```
...
```JavaScript
Boolean RangeStream.emit(Value ev,
    ...args);
```
...
```JavaScript
event RangeStream.data(Buffer data);
```
...
```JavaScript
event RangeStream.error(Integer code);
```
