...
```JavaScript
readonly XmlElement XmlNode.parentElement;
```
...
```JavaScript
readonly Boolean XmlNode.isConnected;
```
...
```JavaScript
Boolean XmlNode.contains(XmlNode node);
```
...
```JavaScript
Integer XmlNode.compareDocumentPosition(XmlNode other);
```

调用参数:
* other: XmlNode, 要比较的节点
...
```JavaScript
Boolean XmlNode.isEqualNode(XmlNode other);
```
...
```JavaScript
Boolean XmlNode.isSameNode(XmlNode other);
```
